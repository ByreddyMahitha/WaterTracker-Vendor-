...
index <- createDataPartition(ModelDataFactored$Preferred, p = 0.85, list = FALSE)
training <- ModelDataFactored[index,]
testing <- ModelDataFactored[-index,]

decTree <- rpart(Preferred~., data = training, method = "class", control = rpart.control(cp = 0.0000001))
...
table2 <- table(predVector2, testing$Preferred)
